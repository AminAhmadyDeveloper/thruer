  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          <svg
            className="size-6 stroke-2 stroke-current"
            fill="none"
            strokeLinecap="round"
      ...
      <DropdownMenuContent>
        <DropdownMenuRadioGroup
          ...
          value={locale}
        >
          <For each={routing.locales}>
            {(localeItem) => (
              <DropdownMenuRadioItem key={localeItem} value={localeItem}>
                {convertLanguageCodeToWord(localeItem)}
              </DropdownMenuRadioItem>
            )}
          </For>
        </DropdownMenuRadioGroup>
